...
@interface CPADetailViewController ()

@end

@implementation CPADetailViewController{
...
//for passing object from DraftView to detailView
@synthesize passingReportObj;
...
- (void)viewDidLoad {
    ...
    categoryField.text = passingReportObj.category;
    
    
    //for timeField & UIDatePickerView
    // your label text is set to current time
    timeField.text= passingReportObj.time;
    ...
    forAddressText.text = passingReportObj.location;
    commentField.text = passingReportObj.comments;

    latitudeLabel.text = passingReportObj.latitude;
    longitudeLabel.text = passingReportObj.longitude;
    
    imageView.contentMode = UIViewContentModeScaleAspectFit;
    imageView.clipsToBounds = YES;
    [imageView setImage:[self loadImage:passingReportObj.imageLocation]];
...
-(IBAction)saveButtonPressed:(id)sender{
    if ([categoryField.text isEqualToString:@"Please select a category"]) {
        ...
        //first remove the old report

//        UIImage *tempImage = [self loadImage:passingReportObj.imageLocation];
        //Create a CPAReport Object.
        CPAReport *reportObj = [[CPAReport alloc] initWithPrimaryKey:0];
       // reportObj.reportID = passingReportObj.reportID;
        NSDateFormatter *dateFormatter3 = [[NSDateFormatter alloc] init];
        [dateFormatter3 setDateFormat:@"MM/dd hh:mm:ss a"];
        NSString *currentTime2 = [dateFormatter3 stringFromDate:[NSDate date]];

        reportObj.date = currentTime2;// when saving, date doesnt matter
        ...
        imageName = [imageName stringByReplacingOccurrencesOfString:@"/" withString:@"-"];
        imageName = [imageName stringByReplacingOccurrencesOfString:@":" withString:@"-"];
        reportObj.imageLocation = [self saveImage:imageView.image forPerson:imageName];

        reportObj.isDirty = NO;
        reportObj.isDetailViewHydrated = YES;
    
        //Add the object
        //  [appDelegate saveReport:reportObj];
        NSLog(@"DELETE: %@ ",passingReportObj.imageLocation);
        [appDelegate removeReport:passingReportObj];
        ...
        //TODO check retrieve
        //    reportObj = [appDelegate.reportArray objectAtIndex:0];
        //     NSLog([NSString stringWithFormat:@"%@, %@",reportObj.category, reportObj.time]);
...
- (void)updatingSuccessfully
{
	UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"Updating successfully!" message:@"You can check from draft" delegate:self cancelButtonTitle:@"OK" otherButtonTitles: nil];
	[alert show];
}
...
-(void)sendButtonPressed:(id)sender{
    ...
    if ([errMessage isEqualToString:@"Please finish the information in the required field"]) {
        ...
        NSString *currentTime2 = [dateFormatter2 stringFromDate:[NSDate date]];
        historyObj.date = currentTime2;
        ...
        imageName = [imageName stringByReplacingOccurrencesOfString:@"/" withString:@"-"];
        imageName = [imageName stringByReplacingOccurrencesOfString:@":" withString:@"-"];
        historyObj.imageLocation = [self saveImage:imageView.image forPerson:imageName];
        ...
        //send to the server:
        //1st: upload the image to server then get URL(when there is a image to upload)
